<template>
  <div>
    ...
    <div class="d-flex gap-2 mb-4 flex-wrap">
      <v-text-field
        v-model="search" prepend-inner-icon="mdi-magnify" label="Angebotsnr. oder Kunde"
        clearable variant="outlined" density="compact" style="max-width:320px"
        @update:model-value="onSearch"
      />
      <v-select
        v-model="statusFilter" :items="statusOptions" label="Status" variant="outlined"
    ...
    <v-card>
      <v-table>
        ...
        <tbody>
          <tr v-if="loading">
            <td colspan="7" class="text-center pa-4"><v-progress-circular indeterminate size="24" /></td>
          </tr>
          <tr v-else-if="!items.length">
            <td colspan="7" class="text-center pa-4 text-medium-emphasis">Keine Angebote gefunden</td>
          </tr>
          <tr v-for="q in items" :key="q.id" style="cursor:pointer" @click="openEditor(q.id)">
            ...
            <td>
              <v-chip :color="STATUS_COLORS[q.status]" size="small" label>
                {{ STATUS_LABELS[q.status] }}
              </v-chip>
            </td>
            <td class="text-right"><strong>{{ fmtEur(q.total) }}</strong></td>
            <td class="text-right" @click.stop>
              <v-btn icon size="small" variant="text" :to="{ name: 'quote-edit', params: { id: q.id } }">
                <v-icon>mdi-pencil</v-icon>
              </v-btn>
              <v-btn icon size="small" variant="text" @click="doDuplicate(q)">
                <v-icon>mdi-content-copy</v-icon><v-tooltip activator="parent">Kopieren</v-tooltip>
              </v-btn>
              <v-btn v-if="q.status === 'draft'" icon size="small" variant="text" color="error" @click="askDelete(q)">
    ...
    <ConfirmDialog ref="confirmRef" title="Angebot löschen?"
      :message="`Angebot ${deleteTarget?.quote_no} wirklich löschen?`" @confirm="doDelete" />
  </div>
</template>
...
const statusOptions = [
  ...
  { title: 'Angenommen', value: 'accepted' },
...
async function load() {
  ...
  try {
    const res = await quotesApi.list({
      skip: (page.value - 1) * LIMIT, limit: LIMIT,
...
function fmtDate(d: string) { return new Date(d).toLocaleDateString('de-DE') }
function fmtEur(v: string) { return `${Number(v).toFixed(2)} €` }

async function doDuplicate(q: Quote) {
  try { await quotesApi.duplicate(q.id); load() } catch (e) { console.error(e) }
}
